const app = new Vue({
    ...
    methods: {
        ...
        async parseTemp() {
            for (let index = 0; index < app.temp.length; index++) {
                ...
                if (app.temp[index].name.charAt(0) == ' ') {
                    app.temp[index].name = app.temp[index].name.substring(1, app.temp[index].name.lenght);
                }
                ...
                try {
                    ...
                    app.temp[index].backdrop = 'https://image.tmdb.org/t/p/original' + app.tmdb[index].backdrop_path;
        ...
        removeDuplicate(films) {
            newFilm = films.filter(film => film.id != '1gHH64FDRYoSfP3y6wh0Vf051qYA5CXn_' &&
            ...
            for (let index = 0; index < app.cards.length; index++) {
                if (app.cards[index].name.indexOf('(Part 1)') != -1) {
                    for (let element of app.cards) {
                        if (app.cards[index].name.replace('(Part 1)', '') === element.name.replace('(Part 2)', '')) {
                            app.cards[index].id2 = element.id;
                            app.cards[index].name = app.cards[index].name.replace('(Part 1)', '');
                        }
                    }
                }
            }
        ...
        async addInfo() {
            for (let index = 0; index < app.cards.length; index++) {
                ...
                await this.updateCall(app.cards[index].name, app.cards[index].year, index);
                try {
        ...
        download() {
            ...
            const file = new Blob([JSON.stringify(app.cards)], { type: 'text/plain' });
        ...
        editInfo(index) {
            if (app.search_flag == true) {
                ...
                document.getElementById('image').value = app.cards[index].posterpath;
            }
        },
        update() {
            app.temp = app.cards;
        ...
        async updateAuto() {
            ...
            localStorage.clear();
            app.tmdb = [];
            const x = prompt('Quale film vuoi cercare?', document.getElementById('title').value);
            const y = prompt('Anno di Uscita?', document.getElementById('year').value);
        ...
        checkArrayTmdb(indexResponse, length) {
            if (length == 1) {
                try {
                    ...
                    app.temp[app.index].year = app.tmdb[indexResponse].release_date.substring(0, 4);
                    app.temp[app.index].overview = app.tmdb[indexResponse].overview.substring(0, 160) + '...';
                    app.temp[app.index].posterpath = 'https://image.tmdb.org/t/p/w500' + app.tmdb[indexResponse].poster_path;
                    app.temp[app.index].backdrop = 'https://image.tmdb.org/t/p/original' + app.tmdb[indexResponse].backdrop_path;
                ...
                for (let index = 0; index < app.tmdb.length; index++) {
                    console.log(index, app.tmdb[index].title, app.tmdb[index].release_date);
                }
        ...
        hover(obj) {
            ...
            document.getElementById(obj.name).style.backgroundImage = "url(" + obj.backdrop + ")";
